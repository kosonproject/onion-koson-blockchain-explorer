...
Currently available Koson blockchain explorers have several limitations which are of 
special importance to privacy-oriented users:
 ...
 - do not support Koson testnet nor stagenet networks,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Koson Blockchain Explorer. The example not only shows how to use 
Koson C++ libraries, but also demonstrates how to use:
...
## Onion Koson Blockchain Explorer features
 ...
 - showing public components of Koson addresses,
 - decoding which outputs and mixins belong to the given Koson address and viewkey,
 - can prove that you send Koson to someone,
 - detailed information about mixins, such as, mixins' age, timescale, mixin of mixins,
 - showing number of amount output indices,
 - support Koson testnet, stagenet network,
...
Current development branch:
 ...
 - https://github.com/kosonproject/onion-koson-blockchain-explorer/tree/koson



## Compilation on Ubuntu 16.04/18.04

##### Compile latest Koson master version

Download and compile recent Koson into your home folder:

```bash
# first install Koson dependecines
sudo apt update
...
Once the Koson is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/Koson
cd ~

# download the source code
git clone https://github.com/kosonproject/onion-koson-blockchain-explorer.git

# enter the downloaded sourced code folder
cd onion-koson-blockchain-explorer
...
# alternatively can use: cmake -DKOSON_DIR=/path/to/koson ..
# if koson is not in ~/koson
#
...
By default it will look for blockchain in its default location i.e., `~/.koson/lmdb`.
...
```
lokblocks, Onion Koson Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Koson total emission monitoring
                                        thread
...
alias lokblocksmainnet='~/onion-koson-blockchain-explorer/build/lokblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias lokblockstestnet='~/onion-koson-blockchain-explorer/build/lokblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Koson emission 

Obtaining current Koson emission amount is not straight forward. Thus, by default it is 
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.koson/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.koson/testnet/lmdb/emission_amount.txt` or `~/.koson/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.koson/lmdb/emission_amount.txt`
...
```
Koson emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Koson daemon's  `print_coinbase_tx_sum` command. 
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_koson",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "koson_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Koson.
...
Return raw json tx data, as represented in Koson.
