...
show_service_node(const std::string &service_node_pubkey)
{
    ...
    else
    {
      ...
      snprintf(buf, sizeof(buf),
          "This service node is awaiting to be registered and has: %s KSN to be contributed remaining",
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Koson address not provided!");
    }
    ...
    // parse string representing given Koson address
    ...
    if (!lokeg::parse_str_address(lok_address_str,  address_info, nettype))
    {
        ...
        return string("Cant parse Koson address: " + lok_address_str);
    }
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_koson_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
search(string search_text)
{
    ...
    // check if koson address is given based on its length
    ...
    // check if integrated koson address is given based on its length
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Koson address not provided";
        return j_response;
    }
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Koson address: " + address_str;
        return j_response;
...
json_networkinfo()
{
    ...
    if (!get_koson_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get Koson network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"koson_version_full"   , string {KOSON_VERSION_FULL}},
...
bool
get_koson_network_info(json& j_info)
{
