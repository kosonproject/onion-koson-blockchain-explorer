...
bool
rpccalls::connect_to_koson_daemon()
{
...
rpccalls::get_current_height()
{
    ...
    if (!r)
    {
        cerr << "Error connecting to Koson daemon at "
             << daemon_url << endl;
        return 0;
    }
...
rpccalls::get_network_info(COMMAND_RPC_GET_INFO::response& response)
{
    ...
    if (r)
    {
        ...
        if (!err.empty())
        {
            cerr << "Error connecting to Koson daemon due to "
                 << err << endl;
            return false;
        }
    }
...
rpccalls::get_block(string const& blk_hash, block& blk, string& error_msg)
{
    ...
    {
        cerr << "get_block: error connecting to Koson daemon at "
             << daemon_url << endl;
        return false;
    }
...
rpccalls::get_service_node(COMMAND_RPC_GET_SERVICE_NODES::response &res, const std::vector<std::string> &pubkeys)
{
    ...
    if (!result)
        cerr << "Error connecting to Koson daemon at " << daemon_url << endl;
